...
import { UtentedatiComponent } from '../utentedati/utentedati.component';

@Component({
  selector: 'app-userpage',
  standalone: true,
  imports: [FormsModule, CommonModule, UtentedatiComponent],
...
export class UserpageComponent implements OnInit {
  ...
  async addTask(): Promise<void> {
    ...
    if (
      ...
      !this.newTask.dataInizio ||
      !this.newTask.dataFine ||
      !this.newTask.tipoLavoro
    ) {
  ...
  async loadTask(): Promise<void> {
    try {
      this.tasks = await firstValueFrom(
        this.taskService.getTasksByUtente(this.utenteId)
      );
    } catch (error) {
  ...
  async getResolvedTasks(): Promise<void> {
    try {
      this.tasks = await firstValueFrom(
        this.taskService.getResolvedTasks(this.utenteId)
      );
    } catch (error) {
  ...
  async getUnresolvedTasks(): Promise<void> {
    try {
      this.tasks = await firstValueFrom(
        this.taskService.getUnresolvedTasks(this.utenteId)
      );
    } catch (error) {
